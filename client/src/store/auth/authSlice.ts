...
export const forgotSendEmail = createAsyncThunk(
  ...
  async (email: string, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_URL}${endpoints.auth.forgotSendEmail}`,
        { email },
        { headers: { "Content-Type": "application/json" } }
      );
      if (response.data?.message) {
  ...
  async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        const msg =
          error.response?.data?.message?.password ||
          error.response?.data?.message?.confirmPassword ||
          error.response?.data?.message ||
          "Registration failed";
        toast.error(msg);
        return rejectWithValue(msg);
...
export const { resetLoading, setEmail, reSetDetails } = authSlice.actions;
